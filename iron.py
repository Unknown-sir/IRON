    ...
    async def send_control(self, ftype: int, sid: int, payload: bytes = b"") -> None:
        ...
        async with self.control_lock:
            if self.control_writer is not writer or writer.is_closing():
                raise ConnectionError("control channel changed or closed")
            await asyncio.wait_for(send_frame(writer, ftype, sid, payload), timeout=cfg_int(self.cfg, "send_timeout_seconds", 15))
